...
def handler(db: Session, task: SyncTask) -> dict[str, Any]:
    """Ingest Lambda functions into `resource_snapshots`."""
    ...
    ing = ingestion_service.ingest_lambda(db, tenant_id, cloud_account_id)
    return {"collector": "lambda", "ingested": ing.ingested_count}
