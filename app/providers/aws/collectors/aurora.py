...
def handler(db: Session, task: SyncTask) -> dict[str, Any]:
    """
    Aurora-specific collector scaffold.

    Phase-1: reuse the existing `ingest_rds` implementation (it already includes Aurora clusters).
    """
    ...
    ing = ingestion_service.ingest_rds(db, tenant_id, cloud_account_id)
    return {"collector": "aurora", "ingested": ing.ingested_count}
