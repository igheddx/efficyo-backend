...
def _is_placeholder_account(ca: CloudAccount) -> bool:
    """Return True if this is an incomplete CFN onboarding session with no real AWS account ID.

    Placeholder account_ids are 12-char hex strings (a-f0-9) generated by _placeholder_account_id().
    Real AWS account IDs are strictly 12 decimal digits.
    """
    aid = ca.account_id or ""
    return bool(ca.onboarding_token) and not aid.isdigit()
...
def list_cloud_accounts(db_session: Session, tenant_id: UUID) -> list[CloudAccount]:
    ...
    # Filter out incomplete CFN onboarding sessions (placeholder account_id not yet confirmed).
    rows = [r for r in rows if not _is_placeholder_account(r)]
    if rows:
        return rows
